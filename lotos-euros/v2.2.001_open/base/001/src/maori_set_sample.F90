...
contains
      ...
      select case ( trim(obs_type) )
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~              
          ...
          obs_allow_missing = .false.
            ...
            IF_ERROR_RETURN(status=1)
	   ! Santiago extract Loclist 
	      !call goVarValue( mdata%loclist, ';', 'file', '=', fname, status )
          ...
          if ( .not. exist ) then
            ...
            !! testing ...
            write (gol,*) ' Santiago xxx open obs_csv file ', trim(fname); call goPr
            ! open new data file:
            ...
            do
              ...
              write (gol,*) 'Santiago  x readrecord ', val, ' ', trim(unit), ' ', status; call goPr
	      write (*,*) 'Santiago  x  status 1',status
              if ( status < 0 ) then
                ! end of file:
                exit
              ...
              call MAORI_LocList_Find( mdata%loclist, iloc, status, &
                                          code=trim(station_code), quiet=.false. )
	     write (*,*) 'Santiago  x  status 2',status
	     write(*,*) 'Santiago x Code', trim(station_code)
              ...
              !! testing ...
              write (gol,*) 'Santiago  x   loc ', trim(station_code), ' value ',  val, ' ', trim(unit); call goPr
  ...
  subroutine MAORI_Set_Output_State_Create( moutput, mdata, t, status )
    ...
    !! testing ...
    write (gol,*) 'Santiago xxx nloc = ', mdata%nloc; call goPr
    !TRACEBACK; status=1; return
