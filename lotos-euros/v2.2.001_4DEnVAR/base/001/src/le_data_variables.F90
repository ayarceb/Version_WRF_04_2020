    ...
    if ( status == 0 ) then
      ...
      if ( len_trim(V%routine) > 0 ) then
        ...
        select case ( trim(V%routine) )
          ...
          case ( 'Total' )
            ! arguments: ( *_met )
            call Variables_Calc_Total( self, trim(varname), V%deps(1:V%ndep), tmid, status )
            IF_NOTOK_RETURN(status=1) 
...
!  end subroutine Variables_Calc_MixlayerHAltBounds
  ...
  subroutine Variables_Calc_Total( self, varname, args, t, status )
    ...
    ! pointer to target field given the name, obtain target units and bounds:
    call self%GetVariable( varname, status, Instant_Field=iF, &
                              check_units=units, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)

    ! set level index space if not defined yet:
    if ( ubo(3) < lbo(3) ) then
      ! pointer to source array santiago:
      call self%GetValidPtr( args(1), t,t, values, status )
      IF_NOTOK_RETURN(status=1)
      ! copy level bounds:
      lbo(3) = lbound(values,3)
      ubo(3) = ubound(values,3)
    end if
    
    ! get pointer to target array (allocate if necessary), set target time:
    call iF%GetTargetPtr( total, t, status, lbo=lbo, ubo=ubo )
    IF_NOTOK_RETURN(status=1)
    ...
    do ivar = 1, size(args)
      ...
      ! pointer to source array Santiago:
      call self%GetValidPtr( args(ivar),t, t, values, status, check_units=units, &
                               check_lbo=lbo, check_ubo=ubo )
      IF_NOTOK_RETURN(status=1)
      ...
      do k = lbo(3), ubo(3)
        do j = lbo(2), ubo(2)
          do i = lbo(1), ubo(1)
            ! add contribution:
            total(i,j,k) = total(i,j,k) + values(i,j,k)
          end do ! i
        end do ! j
      end do ! k
  ...
  end subroutine Variables_Calc_Total
  ...
  !
  ! calculate temperature from potential temperature using
  ! reference temperature, reference pressure and kappa:
  ! 
  ...
  subroutine T_from_potentialtemperature( self, varname, args, t, status )
    ...
    ! get pointer to half-level pressure to obtain number of levels if necessary:
    call self%GetValidPtr( args(i_p), t,t, p, status, &
    ...
    ! compute height profile ;
    ! zero pressure is at infinite distance, so use minimum value:
    temperature= (tpot+Tbase) * (p/p0)**(kappa)
